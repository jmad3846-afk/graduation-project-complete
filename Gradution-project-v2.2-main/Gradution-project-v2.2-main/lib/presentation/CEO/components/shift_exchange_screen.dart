...
class ShiftExchangePage extends StatelessWidget {
  ...
  void _handleAccept(BuildContext context, int id, String sender) {
    // منطق قبول الطلب (تحديث الحالة في قاعدة البيانات)
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم قبول طلب التبديل من $sender')));
    print('Accepted request $id');
  }

  void _handleReject(BuildContext context, int id, String sender) {
    // منطق رفض الطلب (تحديث الحالة في قاعدة البيانات)
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم رفض طلب التبديل من $sender')));
      ...
      child: Scaffold(
        ...
        body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final message = 'طلب تبديل مع ${request['sender']} مناوبة ${request['shift']} مركز ${request['center']}';
